...
def get_chat_completion(input_messages, model_name=gpt4oclient):
    try:
        ...
        elif model_name == gpt4oclient:
            print(f"Model Used: {gpt4oclient}")
            response = gpt4oclient.chat.completions.create(
                model=model_name,
                messages=input_messages)
...
def compare_text(specs_text, user_text):
    ...
    embedding1 = generate_embedding(gpt4oclient, specs_text)
    embedding2 = generate_embedding(gpt4oclient, user_text)
...
def refine_external(response, input_xml, transformed_xml, main_xslt,chat_history):
    ...
    gpt_response = get_chat_completion(prompt, gpt4oclient)
    show_stats(gpt_response)
    complete_response = gpt_response.__str__()
    print(f"COMPLETE_RESPONSE: {complete_response}")
    response = gpt_response.choices[0].message.content
